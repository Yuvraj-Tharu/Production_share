...
import AccreditationServices from '../services/accreditation_services';
...
class BrandController {
  static async getAllBrand(req: Request, res: Response): Promise<void> {
    try {
      ...
      const brand = await AccreditationServices.getAllBrand(
        matchData,
        sortData,
        Number(page),
        Number(perPage),
      ...
      if (!brand) {
        res.status(404).json(apiError('Accreditation not found', {}, 404));
        return;
      }
      res
        .status(200)
        .json(success('Accreditation fetched successfully', 200, brand));
    } catch (error: any) {
  ...
  static async createBrand(req: Request, res: Response): Promise<void> {
    try {
      const brand = await AccreditationServices.createBrand(req.body);
      if (!brand) {
      ...
      res
        .status(201)
        .json(success('Accreditation created successfully', 201, brand));
    } catch (error: any) {
  ...
  static async getBrand(req: Request, res: Response): Promise<void> {
    try {
      const brand = await AccreditationServices.getBrandById(req.params.id);
      if (!brand) {
        res.status(404).json(apiError('Accreditation not found', {}, 404));
        return;
      }
  ...
  static async deleteBrand(req: Request, res: Response): Promise<void> {
    try {
      const brand = await AccreditationServices.deleteBrand(req.params.id);
      if (!brand) {
        res.status(404).json(apiError('Accreditation not found', {}, 404));
        return;
      }
      res
        .status(200)
        .json(success('Accreditation deleted successfully', 200, brand));
    } catch (error: any) {
  ...
  static async updateBrand(req: Request, res: Response): Promise<void> {
    try {
      const brand = await AccreditationServices.updateBrand(
        req.params.id,
        req.body,
      );
      ...
      res
        .status(200)
        .json(success('Accreditation updated successfully', 200, brand));
    } catch (error: any) {
