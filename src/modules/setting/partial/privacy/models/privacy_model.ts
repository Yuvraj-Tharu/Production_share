...
privacySchema.plugin(mongooseDelete, {
  deletedAt: true,
  overrideMethods: 'all',
});
...
privacySchema.plugin(schemaMetadataPlugin, {
  defaultComponentOverrides,
  refFieldMapping,
});

privacySchema.statics.getTableFields = function () {
  return ['title'];
};

privacySchema.statics.getSingleInstanceState = function () {
  return true;
};
...
interface IPrivacyModel extends PaginateModel<IPrivacy> {
  getFieldMetadata: () => Record<string, any>;
  getTableFields: () => string[];
  getSingleInstanceState: () => boolean;
}

const Privacy = model<IPrivacy, IPrivacyModel>('Privacy', privacySchema);
